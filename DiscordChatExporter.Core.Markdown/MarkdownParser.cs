...
namespace DiscordChatExporter.Core.Markdown
{
    ...
    public static class MarkdownParser
    {
        ...
        private static readonly RegexParser<Node> BoldFormattedNodeParser =
            new RegexParser<Node>(new Regex("\\*\\*(.+?)\\*\\*(?!\\*)", RegexOptions.Singleline),
        ...
        private static readonly RegexParser<Node> ItalicFormattedNodeParser =
            new RegexParser<Node>(new Regex("\\*(?!\\s)(.+?)(?<!\\s|\\*)\\*(?!\\*)", RegexOptions.Singleline),
        ...
        private static readonly RegexParser<Node> ItalicAltFormattedParser =
            new RegexParser<Node>(new Regex("_([^_]+?)_(?!\\w)", RegexOptions.Singleline),
        ...
        private static readonly RegexParser<Node> ItalicBoldFormattedNodeParser =
            new RegexParser<Node>(new Regex("\\*(\\*\\*(?:.+?)\\*\\*)\\*(?!\\*)", RegexOptions.Singleline),
                m => new FormattedNode(m.Value, "*", TextFormatting.Italic, Parse(m.Groups[1].Value)));

        // Capture until the earliest double underscore not followed by an underscore
        private static readonly RegexParser<Node> UnderlineFormattedNodeParser =
            new RegexParser<Node>(new Regex("__(.+?)__(?!_)", RegexOptions.Singleline),
        ...
        private static readonly RegexParser<Node> ItalicUnderlineFormattedNodeParser =
            new RegexParser<Node>(new Regex("_(__(?:.+?)__)_(?!_)", RegexOptions.Singleline),
                m => new FormattedNode(m.Value, "_", TextFormatting.Italic, Parse(m.Groups[1].Value)));

        // Strikethrough is safe
        private static readonly RegexParser<Node> StrikethroughFormattedNodeParser =
            new RegexParser<Node>(new Regex("~~(.+?)~~", RegexOptions.Singleline),
                m => new FormattedNode(m.Value, "~~", TextFormatting.Strikethrough, Parse(m.Groups[1].Value)));

        // Spoiler is safe
        private static readonly RegexParser<Node> SpoilerFormattedNodeParser =
            new RegexParser<Node>(new Regex("\\|\\|(.+?)\\|\\|", RegexOptions.Singleline),
        ...
        private static readonly RegexParser<Node> InlineCodeBlockNodeParser =
            new RegexParser<Node>(new Regex("`\\s*([^`]+?)\\s*`", RegexOptions.Singleline),
        ...
        private static readonly RegexParser<Node> MultilineCodeBlockNodeParser =
        ...
        // @everyone or @here
        private static readonly RegexParser<Node> MetaMentionNodeParser =
            new RegexParser<Node>(new Regex("@(everyone|here)"),
                m => new MentionNode(m.Value, m.Groups[1].Value, MentionType.Meta));

        // <@123456> or <@!123456>
        private static readonly RegexParser<Node> UserMentionNodeParser =
            new RegexParser<Node>(new Regex("<@!?(\\d+)>"),
                m => new MentionNode(m.Value, m.Groups[1].Value, MentionType.User));

        // <#123456>
        private static readonly RegexParser<Node> ChannelMentionNodeParser =
            new RegexParser<Node>(new Regex("<#(\\d+)>"),
                m => new MentionNode(m.Value, m.Groups[1].Value, MentionType.Channel));

        // <@&123456>
        private static readonly RegexParser<Node> RoleMentionNodeParser =
            new RegexParser<Node>(new Regex("<@&(\\d+)>"),
        ...
        private static readonly RegexParser<Node> StandardEmojiNodeParser =
        ...
        private static readonly RegexParser<Node> CustomEmojiNodeParser =
            new RegexParser<Node>(new Regex("<(a)?:(.+?):(\\d+)>"),
                m => new EmojiNode(m.Value, m.Groups[3].Value, m.Groups[2].Value, !m.Groups[1].Value.IsEmpty()));

        /* Links */

        // [title](link)
        private static readonly RegexParser<Node> TitledLinkNodeParser =
            new RegexParser<Node>(new Regex("\\[(.+?)\\]\\((.+?)\\)"),
        ...
        private static readonly RegexParser<Node> AutoLinkNodeParser =
            new RegexParser<Node>(new Regex("(https?://\\S*[^\\.,:;\"\'\\s])"),
                m => new LinkNode(m.Value, m.Groups[1].Value));

        // Autolink surrounded by angular brackets
        private static readonly RegexParser<Node> HiddenLinkNodeParser =
            new RegexParser<Node>(new Regex("<(https?://\\S*[^\\.,:;\"\'\\s])>"),
                m => new LinkNode(m.Value, m.Groups[1].Value));
        ...
        private static readonly RegexParser<Node> ShrugTextNodeParser =
            new RegexParser<Node>(new Regex(Regex.Escape("¯\\_(ツ)_/¯")),
        ...
        private static readonly RegexParser<Node> EscapedTextNodeParser =
            new RegexParser<Node>(new Regex("\\\\([^a-zA-Z0-9\\s])"),
                m => new TextNode(m.Value, m.Groups[1].Value));

        // Text node encapsulates text not matched by other parsers
        private static readonly RegexParser<Node> FallbackTextNodeParser =
            new RegexParser<Node>(new Regex(".+", RegexOptions.Singleline),
                m => new TextNode(m.Value));

        private static readonly RegexAggregator<Node> NodeParser = new RegexAggregator<Node>(
        ...
        public static IReadOnlyList<Node> Parse(string input) => NodeParser.Parse(input);
    }
}